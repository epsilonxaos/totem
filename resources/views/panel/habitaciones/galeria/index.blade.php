		...
		<div class="max-w-7xl mx-auto">
			<div class="flex items-center md:justify-between mb-8">
				<div class="w-full md:w-auto">
					<h2 class="mb-2 font-semibold uppercase text-gray-900 text-base">Galeria de imágenes</h2>
				</div>
				<div class="w-full md:w-auto">
					@if ($accion === 'edit')
			...
			<div class="bg-white pt-0 mb-8">
				<p> Si no desea incluir más imágenes, pulse el botón <span class="text-blue-800 font-semibold">"Guardar y finalizar"</span>.</p>
				<p class="mb-5 text-base">Solo se permite subir un <span class="text-yellow-500 font-semibold">máximo de 20 imágenes por lote</span>. agregar más imágenes, simplemente haga clic en el botón "<span class="text-green-700 font-semibold">Guardar imagenes</span>" y podrá seguir subiendo.</p>
				<ul class="text-sm">
					<li>* Las medidas <span class="text-blue-800 font-semibold">recomendadas</span> son 1200 x 670px</li>
				...
				<form action="{{route('panel.habitaciones.galeria.store')}}" method="POST" enctype="multipart/form-data" id="my-dropzone-galeria" class="dropzone mt-3" style="border: 2px dashed #d6d6d6;">
					@csrf
				...
				<div class="container">
					...
					<div class="row">
						...
						<div class="w-full">
							<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 sortable-items" data-url="{{route('panel.habitaciones.galeria.ordenamiento')}}">
									@foreach ($galeria as $item)
										<div class="mb-3 relative sort px-2" data-orden="{{$item -> order}}" data-idx="{{$item -> id}}">
											...
											<img src="{{asset($item -> cover)}}" class="w-full h-[120px] object-contain" alt="Base">
										</div>
									@endforeach
								</div>
							</div>
			...
			<div class="w-full md:w-auto">
				<h2 class="mb-2 font-semibold uppercase text-gray-900 text-base">Amenidades de la habitación</h2>
			</div>
			<div class="bg-white pt-4">
				<ul class="text-sm">
					<li>* Las medidas <span class="text-blue-800 font-semibold">recomendadas</span> son 100 x 100px</li>
					<li>* Solo se aceptan los formatos <span class="text-blue-800 font-semibold">.svg</span></li>
					<li>* Maximo de peso de <span class="text-yellow-500 font-semibold">200KiB</span>.</li>
				</ul>
				<form action="{{route('panel.habitaciones.amenidades.store')}}" method="POST" enctype="multipart/form-data" id="my-dropzone-amenidades" class="dropzone mt-3" style="border: 2px dashed #d6d6d6;">
					@csrf
					<input type="hidden" name="id" value="{{$id}}">
					<div class="dz-message">
						Suelta tus archivos aquí
				...
				<div class="container">
					@if (count($amenidades) > 0)
					<div class="row">
						<div class="w-full py-4"> <hr> </div>
						<div class="w-full">
							<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 sortable-items" data-url="{{route('panel.habitaciones.amenidades.ordenamiento')}}">
									@foreach ($amenidades as $item)
										<div class="mb-3 relative sort px-2" data-orden="{{$item -> order}}" data-idx="{{$item -> id}}">
											<div class="absolute z-10 top-0 right-0 flex items-center justify-content-end pr-3">
												...
												<div class="item bg-red-800 text-white delete delete-axios cursor-pointer" data-url="{{route('panel.habitaciones.amenidades.destroy')}}" data-idx="{{$item -> id}}" title="Eliminar">
													<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#ffffff" d="m20.37 8.91l-1 1.73l-12.13-7l1-1.73l3.04 1.75l1.36-.37l4.33 2.5l.37 1.37l3.03 1.75M6 19V7h5.07L18 11v8a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2Z"/></svg>
												</div>
											</div>
											<img src="{{asset($item -> cover)}}" class="w-full h-[120px] object-contain" alt="Base">
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/dropzone/5.9.2/dropzone.min.css" integrity="sha512-jU/7UFiaW5UBGODEopEqnbIAHOI8fO6T99m7Tsmqs2gkdujByJfkCbbfPSN4Wlqlb9TGnsuC0YgUgWkRBK7B9A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
	...
	Dropzone.autoDiscover = false;
       	var g = new Dropzone('#my-dropzone-galeria', {
            paramName: 'file',
            maxFiles: 20,
            maxFilesize: 2,
		...
		var a = new Dropzone('#my-dropzone-amenidades', {
            paramName: 'file',
            maxFiles: 20,
            maxFilesize: .2,
            maxThumbnailFilesize: .2,
