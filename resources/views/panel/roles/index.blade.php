		...
		<tbody>
			...
			<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-emerald-50 dark:hover:bg-gray-600">
				...
				<td class="px-6 py-4 text-center flex items-center justify-center">
					...
					<a title="Editar rol" href="{{ route('panel.roles.edit', ['id' => $item->id]) }}" class="text-white bg-yellow-700 hover:bg-yellow-800 font-medium rounded-lg text-xs px-2 py-1 mr-2 inline-flex items-center justify-center" style="width: 36px;">
					...
					<form action="{{ route('panel.roles.destroy', ['id' => $item->id]) }}" method="post" class="inline delete-form-{{ $item->id }}">
						@csrf
						@method('DELETE')
						<button title="Eliminar" type="button" onclick="deleteSubmitForm({{ $item->id }})" class="text-white bg-red-700 hover:bg-red-800 font-medium rounded-lg text-xs px-2 py-1 mr-2 inline-flex items-center justify-center" style="width: 36px">
